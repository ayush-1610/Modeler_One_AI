...
| S1 IV | fit clearance and distribution to IV data | **Partial — runs on PK-Sim** | round loop build → simulate → evaluate → diagnose → fit; fit judged in its own round; renal clearance rule (diag-rules 0.2); PI with SD/CV/95% CI. enzyme/transporter expression profiles harvested from the OSP models (enzyme clearance proven to act on PK-Sim). PK-Sim-verified 2026-09-24 (Aciclovir, illustrative IV data) | VPC gate (R7); fit starts run sequentially (slow on the single-node runner) | 1.5 |
